...
from unet import *
from model.unetPP import NestedUNet as UnetPlusPlus
...
def inference(args, model, test_save_path=None):
    """
    评估模型性能并记录指标，
    并将输入图和 mask 保存到 test_save_path 父目录下的 imgs/ 和 masks/，
    预测结果直接保存到 test_save_path。
    """
    # --- 在 test_save_path 的上一级目录创建 imgs/ 和 masks/ ---
    if test_save_path:
        parent = os.path.dirname(test_save_path.rstrip("/"))
    ...
    val_loader = DataLoader(db_full, batch_size=1, shuffle=False,
                            num_workers=4, pin_memory=True)

    # 初始化指标存储……
    all_metrics = { k: [[] for _ in range(args.num_classes-1)]
                    for k in ['dice','hd95','precision','sensitivity','iou','volume_similarity'] }
    case_results = []

    model.eval()
    with torch.no_grad():
        for i_batch, batch in tqdm(enumerate(val_loader),
                                   total=len(val_loader), desc="评估中"):
            image, label = batch["image"].cuda(), batch["mask"].cuda()
            case_name = f"case_{i_batch:03d}"

            # —— 先保存输入和 mask —— #
            img_tensor  = image[0].cpu()
            mask_tensor = label[0].cpu().float()
            save_image(img_tensor,  os.path.join(img_dir,  case_name + ".png"))
            save_image(mask_tensor, os.path.join(mask_dir, case_name + ".png"))
            # —— end —— #

            # 获取预测结果和各项指标，pred 直接会写到 test_save_path 下
...
if __name__ == "__main__":
    ...
    # 模型路径（根据训练生成的路径调整）
    args.exp = 'EncoderDoubleCoordAttUNet'
    snapshot_path = "./checkpoints8/checkpoint_epoch100.pth"

    # 加载模型
    input_channels = 3

    net = EncoderDoubleCoordAttUNet(n_channels=3, n_classes=args.num_classes).cuda()
