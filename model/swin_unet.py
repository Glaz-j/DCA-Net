...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, n_classes=1000,
...
class Swin_Unet(nn.Module):
    def __init__(self, img_size, patch_size, in_channels, n_classes,
        ...
        # 添加输出后处理层
        self.output_resize = nn.Sequential(
        ...
        # 添加后处理后的最终卷积
        self.final_output = nn.Conv2d(n_classes, n_classes, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # 输出后处理：调整回原始尺寸
        self.output_resize[1] = nn.Upsample(size=original_size, mode='bilinear', align_corners=False)
